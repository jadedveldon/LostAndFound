      ...
      <main className="flex-1 flex flex-col w-full overflow-hidden">
        ...
        <section className="pt-32 pb-16 section bg-[var(--paper)]">
          <div className="container max-w-4xl text-center">
            <span className="t-eyebrow block mb-6">L&F GATHERINGS · 2026 · 2027</span>
            ...
            {filteredGatherings.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-16 justify-items-center">
                {filteredGatherings.map((g, idx) => (
                  <Link key={idx} href={`/gatherings/${g.slug}`} className="w-full max-w-[360px]">
                    <GatheringCard 
                      title={g.title}
                      location={g.location}
                      dates={g.dates}
        ...
        <section className="w-full py-16 text-center border-t border-[var(--paper-deep)]">
          ...
          <Link href="/experience" className="t-h3-italic text-[var(--clay)] hover:text-[var(--clay-deep)] transition-colors">
